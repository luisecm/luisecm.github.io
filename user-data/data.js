export const bio = [
  "Hello, I'm Luis Cardeña!",
  "Highly experienced <strong>QA Engineer</strong> with over 12 years of expertise in manual and automated testing for web, mobile, and desktop applications. Proven track record in designing and maintaining test automation frameworks, setting up CI/CD pipelines, and leading QA teams.",
  "Skilled in Cypress, Postman, SQL, NoSQL, and various other testing tools. Adept at improving QA processes and ensuring high- quality software delivery",
];

export const skills = [
  "Playwright, Cypress, Appium, WebdriverIO, TypeScript, Javascript",
  "SQL, NoSQL, API",
  "GitHub, GitLab, Git",
];

export const experience = [
  {
    title: "Freelance Work",
    duration: "February 2022 - Present",
    subtitle: "Test Automation Engineer",
    details: [
      "Design, implement and maintain test automation frameworks for Web and Mobile applications using JavaScript/TypeScript and CypressJS, WebdriverIO, Appium and Playwright.",
      "Increase coverage of test suite in order to reduce the time spent during regression, smoke and sanity testing cycles by manual testing team",
      "Setup and maintain CI/CD pipelines for building apps, set up test environment, running automated tests and releasing applications with Github Actions and implemented other workflows to improve quality",
      "Create and execute manual test plans and select and prioritize automation candidate tests",
    ],
    tags: ["Playwright", "Appium", "WebdriverIO", "Cypress"],
    icon: "truck ",
  },
  {
    title: "Encora",
    duration: "January 2021 - February 2022",
    subtitle: "QA Engineer",
    details: [
      "Create, design, execute and maintain manual test plans for web applications",
      "Design framework and strategy for updating a Cypress automation framework for a web application",
      "Automation of API testing with Postman and Cypress",
      "Worked with SQL and NoSQL databases",
      "Contribution to create a high quality governance system for the user actions as well as developing new functionalities to reduce the time previously spent on repetitive tasks",
    ],
    tags: [
      "Cypress",
      "SQL",
      "Postman",
      "TestRail",
      "Git",
      "PostgreSQL",
      "MongoDB",
    ],
    icon: "heartbeat",
  },
  {
    title: "AgileThought",
    duration: "February 2018 - December 2020",
    subtitle: "QA Lead",
    details: [
      "Managed a small team of remote testers and at the same time continued executing my activities as QA Engineer for the team",
      "Trained new members and provided assistance to QA engineers on SQL/Progress scripting and test plan creations",
      "Worked on improvements to existing to tools for ETL process and validation of source data received",
      "Ensured that ETL processes were working as expected to process data received from client companies",
    ],
    tags: ["SQL", "ETL", "OpenEdge", "Data Testing", "QA"],
    icon: "qrcode",
  },
  {
    title: "AgileThought",
    duration: "May 2015 - January 2018",
    subtitle: "QA Engineer",
    details: [
      "Created, maintained and executed test plans for ETL interfaces for one of the top companies on disability claims management",
      "Developed SQL and OpenEdge Progress scripts to validate data processed",
      "Contributed to ensure that HR data from clients was loaded correctly for payment processes executed• Created, maintained and executed test plans for ETL interfaces for one of the top companies on disability claims management • Developed SQL and OpenEdge Progress scripts to validate data processed • Contributed to ensure that HR data from clients was loaded correctly for payment processes executed",
    ],
    tags: ["SQL", "ETL", "OpenEdge", "Data Testing", "QA"],
    icon: "group",
  },
];

export const repos = [
  {
    author: "luisecm",
    name: "luisecm",
    description: "My personal portfolio",
    language: "TypeScript",
  },
];

export const education = [
  {
    title: "Bachelor in Information Technology Management",
    duration: "2005 - 2009",
    subtitle: "Universidad Autónoma de Yucatán",
    details: [],
    tags: [
      "Operating Systems",
      "Database Management System",
      "Computer Networks",
      "Project Management",
      "Programming",
    ],
    icon: "graduation-cap",
  },
  {
    title: "Executive Master in Information Technology Management",
    duration: "2013 - 2014",
    subtitle: "Universidad Tecmilenio",
    details: [],
    tags: ["Programming", "Database Management", "Project Management"],
    icon: "book",
  },
];

export const footer = [
  {
    label: "Links",
    data: [
      {
        text: "Linkedin",
        link: "https://www.linkedin.com/in/luiscardenamaldonado/",
      },
      {
        text: "Github",
        link: "https://github.com/luisecm",
      },
    ],
  },
];
